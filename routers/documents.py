...
# The document-archive flow (soft-delete + restore + smart param re-pointing)
# was disabled per product request: deleting a document now permanently
# removes it AND triggers a full re-extraction from the remaining docs.
# Keeping the code below commented so the feature can be revived later.
#
...
def delete_documents(
    ...
    """Permanently delete selected documents from a project, then re-extract
    parameters from any remaining documents.

    Cleans up Pinecone vectors, extracted parameter references,
    document chunks, BOQ items, and physical files.
    """
    project = db.query(Project).filter(Project.project_id == project_id).first()
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    # ── Step 5: Re-extract parameters from any remaining processed docs ────
    remaining_count = db.query(Document).filter(
        Document.project_id == project_id,
        Document.processed == True,
    ).count()
    re_extracting = remaining_count > 0
    if re_extracting:
        from services.extraction import _run_extraction
    ...
    logger.info(
        ...
        f"{remaining_count} remaining, re_extracting={re_extracting}"
    )
    return {
